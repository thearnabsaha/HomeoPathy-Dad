...
import { useNeoRecentSearches } from "@/hooks/useNeoRecentSearches";
...
export function NeoSearchBar({ onSelectSymptom, onSelectRemedy }: NeoSearchBarProps) {
  const { t, language } = useTranslation();
  const isBn = language === "bn";
  const bn = (s: string) => (isBn ? translateRepertory(s) : s);
  const { searches, addSearch } = useNeoRecentSearches();
  ...
  const doSearch = useCallback(async (q: string) => {
    if (q.length < 2) { setResults({ symptoms: [], remedies: [] }); return; }
    setLoading(true);
    try {
      const [symRes, remRes] = await Promise.all([
        neoApi.searchSymptoms(q),
        neoApi.searchRemedies(q),
      ]);
      setResults({
  ...
  const selectSymptom = (id: string, name: string) => {
    addSearch(name);
    setQuery(""); setIsOpen(false); setMobileOpen(false);
    onSelectSymptom?.(id);
  };
  ...
  const closeMobile = () => { setMobileOpen(false); setQuery(""); setResults({ symptoms: [], remedies: [] }); };
    ...
    <>
      ...
      {showRecent && (
        <div className={large ? "px-4 py-2" : "p-2"}>
          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t("search.recent")}</div>
          {searches.map((s, i) => (
            <button key={i} onClick={() => handleChange(s)}
              className={`flex items-center gap-3 w-full px-3 text-left hover:bg-accent active:bg-accent rounded-lg transition-colors ${large ? "py-3 text-base" : "py-1.5 text-sm"}`}>
              <Clock className="h-4 w-4 text-muted-foreground shrink-0" />{s}
            </button>
          ))}
        </div>
      )}
      {results.symptoms.length > 0 && (
        <div className={large ? "px-4 py-2" : "p-2"}>
          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
            <Stethoscope className="h-3 w-3" />{t("search.symptoms")}
          </div>
          {results.symptoms.map((s) => (
            <button key={s.id} onClick={() => selectSymptom(s.id, s.name)}
              className={`flex flex-col w-full px-3 text-left hover:bg-accent active:bg-accent rounded-lg transition-colors ${large ? "py-3" : "py-1.5"}`}>
              <span className={large ? "text-base" : "text-sm"}>{bn(s.name)}</span>
              <span className="text-xs text-muted-foreground">{bn(s.chapter)}</span>
            </button>
          ))}
        </div>
      )}
      {results.remedies.length > 0 && (
        <div className={large ? "px-4 py-2 border-t border-border" : "p-2 border-t border-border"}>
          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
            <Pill className="h-3 w-3" />{t("search.remedies")}
          </div>
          {results.remedies.map((r) => (
            <button key={r.id} onClick={() => selectRemedy(r.id, r.name)}
              className={`flex items-center gap-3 w-full px-3 text-left hover:bg-accent active:bg-accent rounded-lg transition-colors ${large ? "py-3" : "py-1.5"}`}>
              <span className={large ? "text-base flex-1" : "text-sm"}>{bn(r.name)}</span>
  ...
  const mobileOverlay = mobileOpen && portalRoot
    ? createPortal(
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, background: "hsl(var(--background))", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", borderBottom: "1px solid hsl(var(--border))" }}>
            <button onClick={closeMobile} style={{ padding: "8px", color: "hsl(var(--muted-foreground))", background: "none", border: "none", cursor: "pointer" }}>
              ...
              <div style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 4 }}>
                {query && (
                  <button onClick={() => { setQuery(""); setResults({ symptoms: [], remedies: [] }); mobileInputRef.current?.focus(); }}
                    style={{ padding: 6, color: "hsl(var(--muted-foreground))", background: "none", border: "none", cursor: "pointer" }}>
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-md">
      <div className="relative flex items-center">
        ...
        <input value={query} onChange={(e) => handleChange(e.target.value)} onFocus={() => setIsOpen(true)}
        ...
        <div className="absolute right-1 flex items-center gap-1">
          {query && (
            <button onClick={() => { setQuery(""); setResults({ symptoms: [], remedies: [] }); }} className="p-1 text-muted-foreground hover:text-foreground">
              <X className="h-3 w-3" />
            </button>
          )}
          <VoiceInput onResult={(text) => { setQuery(text); handleChange(text); }} />
      ...
      {isOpen && (hasResults || showRecent || loading || (query.length >= 2 && !hasResults)) && (
